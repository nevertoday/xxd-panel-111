...
# XXD Panel 111｜텍스타일 콜라주 기록

사진에서 가장 기억할 관계를 온기 있는 시각 도장으로 꿰매다.
...
사진의 매력은 정보량보다 관계, 옷의 촉감, 조용한 멈춤에 있을 때가 많습니다. **Panel 111**은 사진의 현실을 유지하고 다른 절반을 올풀림, 천 결, 드러난 바늘땀이 있는 작은 시각 도장으로 다시 꿰맵니다. 인물 관계, 반려동물, 자연, 개인 기억, 출판과 전시에 잘 맞습니다.

촉감 없는 필터, 재료를 과하게 쌓은 콜라주, 값싸 보이는 수공예 효과, 장식에 묻힌 핵심 관계를 해결합니다.
...
**특징:** 선택적 기억 · 손바느질 텍스타일 아플리케 · 드러난 바늘땀 · 원본에서 뽑은 2–4색 천 · 작은 도장형 주체 · 넓은 편집 여백

## 빠른 적합성 확인

| 필요한 것 | Panel 111의 답 |
|---|---|
| 평범한 필터 없이 사진의 온기를 유지 | 현실 사진을 남기고 다른 절반을 천 조각, 올풀림, 바늘땀으로 다시 꿰맵니다. |
| 재료 콜라주가 빽빽하지 않게 | 핵심 관계 하나를 작은 도장으로 만들고 주변을 넉넉히 비웁니다. |
| 값싼 공예 느낌 없는 손맛 | 원본의 2–4색, 실제 섬유, 절제된 바늘땀으로 편집적 결과를 만듭니다. |

## 변환 논리

```text
핵심 주제와 관계 읽기 → 불필요한 세부 삭제 → 구조와 기억점 추출 → 천·올풀림·바늘땀으로 재구성 → 작은 도장과 큰 여백으로 완성
```

## 완성작의 대표 특징

- 현실과 직물 변환은 한 완성 캔버스이며 비교 모드는 정확한 50:50, 세 번째 띠는 없습니다.
- 원본을 식별하는 구조, 흐름, 관계만 남기고 모든 사물 묘사나 배경 채우기를 피합니다.
- 면마 섬유, 실밥, 올풀림, 불규칙 재단, 가장자리 바느질과 작은 어긋남을 디지털 스티커가 아닌 실제 재료처럼 표현합니다.
- 현재 사진의 생명력 있는 2–4색을 추출해 밝은 바탕 천과 균형을 맞춥니다.
- 텍스트는 적고 주체는 치우침, 가장자리 배치, 부유, 부분 자르기가 가능하며 여백도 구성에 참여합니다.
...
표시 언어를 명확히 정합니다. `prompt`는 원문 규칙, `exact`는 사용자 문구 그대로, `none`은 문자·숫자·Logo·가짜 문자를 금지합니다. 인물, 장소, 파일명으로 언어를 추측하지 않습니다.

## 네 가지 출력 모드

- `top-bottom`: 현실 사진을 위, Panel 111 디자인을 아래에 두고 각각 50%입니다.
...
설치 후 Agent 세션을 다시 시작하고 호출하세요 `$xxd-panel-111`.

<!-- xxd-panel-catalog:start -->
## XXD Panel 전체 프로젝트

현재 XXD Panel 시리즈는 001–112까지 업데이트되었으며, 각 Panel은 독립된 원본 프롬프트와 미적 논리를 유지합니다. 아래 표는 이 프로젝트 공개 시점의 과거 목록으로 001–111을 연속해서 나열하며, 현재 프로젝트는 굵게 표시합니다.
