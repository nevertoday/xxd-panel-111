...
```text
读懂核心主题与关系 → 删除无关细节 → 提炼结构与视觉记忆点 → 用布片、毛边与针迹重构 → 以小尺度章印和大留白完成图文海报
```

## 成品中最容易识别的特点

- 真实照片与纺织转译属于同一张完整画布；上下或左右比较都严格 50:50，绝不增加第三带。
- 下半或右半只保留能代表原物的结构、走势和关系，不逐物复制、不填满背景。
- 棉麻纤维、抽丝、毛边、不规则剪裁、锁边针迹和轻微错位是真实材质，不是数字贴图。
- 色彩从当前照片提炼 2–4 种最有生命力的颜色，配浅色底布平衡鲜明色块。
- 文字只做少量编辑性介入；主体可以偏心、贴边、悬置或局部裁切，留白参与构图。
...
111 个 Panel 各自保留独立的原始提示词与审美逻辑。下面连续列出 001–111 的全部项目地址和核心风格特点；当前项目以粗体标出。
