...
- The canonical presentation is a 3:4 portrait canvas with reality above and the textile transformation below, exactly 50:50.
...
Text modes are `prompt`, `exact`, and `none`. Resolve the target locale explicitly. Exact text is passed verbatim; text-free output contains no letters, numbers, logos, labels, or pseudo-text.
...
Write final PNGs directly inside one fresh task directory under `~/Desktop/xxd/xxd-panel-111/` or the explicit output root. Use collision-safe filenames; do not create source, mode, or size subdirectories and do not generate an automatic contact sheet.

Inspect every result at full and thumbnail size. Accept only when the source, ratio, source visibility, split direction, and exact 50:50 midpoint are correct; the transformed region follows the textile-appliqué brief directly from the current source; text follows the chosen mode and locale; and there is no watermark, SVG substitute, UI, third band, or second-pass artefact.
...
- `references/xxd-panel-111-prompt.zh-CN.md` and `.en.md` — delivery adapter notes
