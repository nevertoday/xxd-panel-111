...
# XXD Panel 111｜テキスタイル・コラージュ誌

写真で最も記憶に残る関係を、温度のある視覚の印章へ縫い上げる。
...
写真の魅力は情報量ではなく、関係、衣服の手触り、静かな間に宿ることがあります。**Panel 111** は写真の現実を残し、もう半分を毛羽、布目、見える針目を持つ小さな視覚印章へ縫い直します。人物関係、ペット、自然、私的記憶、出版、展示に向きます。

触覚のないフィルター、素材を詰めすぎたコラージュ、安価に見える手作り表現、装飾に埋もれる主体関係を解決します。
...
**特徴：** 選択的記憶 · 手縫いテキスタイルアップリケ · 見える針目 · 元写真由来の2〜4色 · 小さな印章 · 大きな編集余白

## クイック適性チェック

| 必要なこと | Panel 111 の答え |
|---|---|
| 写真の温度を残し、普通のフィルターを避けたい | 現実写真を残し、もう半分を布片、毛羽、針目で同じ記憶へ縫い直します。 |
| 素材コラージュを詰め込みたくない | 中心的な関係だけを小さな印章にし、周囲を大きく空けます。 |
| 安っぽくない手仕事感がほしい | 2〜4色の布、実在する繊維、抑えた針目で編集的に仕上げます。 |

## 変換ロジック

```text
主題と関係を読む → 不要な細部を削る → 構造と記憶点を抽出 → 布片・毛羽・針目で再構成 → 小さな印章と大余白で完成
```

## 識別しやすい完成特徴

- 写真と繊維変換は一枚の完成画布に属し、比較モードは厳密な50:50で第三帯を作りません。
- 原物を示す構造、流れ、関係だけを残し、全物体の描写や背景の充填を避けます。
- 綿麻繊維、ほつれ、毛端、不規則裁断、かがり縫い、軽いずれをデジタルシールではなく実在素材として見せます。
- 現在の写真から生命力のある2〜4色を選び、淡い底布と調和させます。
- 文字は少量。主体は偏心、接辺、浮遊、部分裁切が可能で、余白も構図として働きます。
...
表示言語を明示し、`prompt` は原文規則、`exact` はユーザー文を逐語、`none` は全文字・数字・Logo・疑似文字を禁止します。人物、場所、ファイル名から言語を推測しません。

## 4つの出力モード

- `top-bottom`：現実写真を上、Panel 111 のデザインを下に、各50%。
...
インストール後にAgentセッションを再起動して呼び出します `$xxd-panel-111`.

<!-- xxd-panel-catalog:start -->
## XXD Panel プロジェクト一覧

XXD Panel の現行シリーズは 001〜112 まで更新され、各 Panel は独立した原文プロンプトと美的ロジックを保持しています。以下は本プロジェクト公開時点の履歴カタログで、001〜111 を連続して掲載し、現在のプロジェクトを太字で示します。
